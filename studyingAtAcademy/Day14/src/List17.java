...
/*	제네릭(지네릭스) 란? 
 *  1.원래 컬렉션은 복수개의 타입을 동시 저장할 수 있다. 그러다 보니 자료의 안정성도 떨어지고 원하는 자료값 추출도 
 *  어렵다. 그리고 불필요한 업 /다운캐스팅 형변환을 해야한다. 이러한 문제점을 해결하기 위해서 자바 5에서 추가된 제네릭
 *  문법이다.
...
/*	문제)1.일반for와 향상for 를 사용하여 'seoul','busan'의 영문대문자로 각각 출력하는 코드를 작성하자.
 */	
		for(int i=0;i<cityList.size();i++) {
			String cityName=cityList.get(i);//get()메서드로 컬렉션에 저장된 원소값을 가져옴
